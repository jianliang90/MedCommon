...
def inference_onecase(network, args, infile):
    ...
    output = output[0].detach().cpu().numpy().squeeze()

    pred_mask_czyx = output
    pred_mask_czyx[output >= 0.5] = 1
    pred_mask_czyx[output < 0.5] = 0
    ...
    data_io.save_medical_info_and_data(pred_mask_zyx, data_dict['origin'], data_dict["spacing"],
                                                     data_dict["direction"], './tmp.nii.gz')
...
if args.mode == "train":
    ...
    inference_onecase(network, args, '/fileser/zhangwd/data/lung/LUNA/RAW_NII/1.3.6.1.4.1.14519.5.2.1.6279.6001.245546033414728092794968890929.nii.gz')
    # inference_onecase(network, args, '/fileser/zhangfan/DataSet/airway_segment_data/train_lung_airway_data/image_refine/ori_128_128_128/1.3.6.1.4.1.14519.5.2.1.6279.6001.148935306123327835217659769212.nii.gz')
